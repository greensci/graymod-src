...
//#endif
	...
	// Store mouse deltas for rotation (captured during ItemPostFrame when we have the command)
	short		m_sStoredMouseDX;
	short		m_sStoredMouseDY;
...
//#ifdef ARGG
	...
	// Mouse deltas are now stored in the weapon during ItemPostFrame
	// Use the stored values instead of trying to access the command here
	if( m_bHasRotatedCarryAngles )
	{
		// Get the weapon to access stored mouse deltas
		CWeaponGravityGun *pWeapon = (CWeaponGravityGun*)(pPlayer->GetActiveWeapon());
		if ( !pWeapon || (pWeapon->m_sStoredMouseDX == 0 && pWeapon->m_sStoredMouseDY == 0) )
		{
		...
		m_vecRotatedCarryAngles[YAW] = pWeapon->m_sStoredMouseDX*0.05;
		m_vecRotatedCarryAngles[PITCH] = pWeapon->m_sStoredMouseDY*-0.05;
		m_vecRotatedCarryAngles[ROLL] = 0;
		
		// Clear the stored deltas after use
		pWeapon->m_sStoredMouseDX = 0;
		pWeapon->m_sStoredMouseDY = 0;
...
//#ifdef ARGG
	...
	if( pObject ) {
		...
		if((pOwner->m_nButtons & IN_USE) ) {
			...
			// Capture mouse deltas while we have access to the command
			CUserCmd *cmd = NULL;
#ifdef CLIENT_DLL
			cmd = pOwner->m_pCurrentCommand;
#else
			cmd = pOwner->GetCurrentCommand();
#endif
			if ( cmd )
			{
				m_sStoredMouseDX = cmd->mousedx;
				m_sStoredMouseDY = cmd->mousedy;
			}
		} else {
